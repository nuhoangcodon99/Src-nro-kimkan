    ...
    private void updateBunma() {
        try {
            if (this.player.setClothes.ctBunmaXecXi != -1 && this.player.zone != null && Util.canDoWithTime(lastTimeXinhQua, 10_000)) {
                ...
                for (Player pl : players) {
                    if (!pl.isMiniPet) {
                        Service.getInstance().chat(pl, "Wow, sexy quá");
                    }
//                    System.out.println("thằng: " + pl.name + " đã kêu");
                }
                this.lastTimeXinhQua = System.currentTimeMillis();
            }
        } catch (Exception e) {
            Log.error("update PunMeo loi" + e.getMessage());
        }
    }
