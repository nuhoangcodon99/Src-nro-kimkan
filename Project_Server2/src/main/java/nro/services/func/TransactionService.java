...
//        }
        if (!pl.getSession().actived) {
            Service.getInstance().sendThongBao(pl, "Bạn chưa phải là thành viên của Ngọc rồng KIMKO");
            return;
        }
        try {
            if (pl.getSession().actived) {
                ...
                Service.getInstance().sendThongBao(pl, "Vui lòng kích hoạt để sử dụng tính năng này!");
            }
        } catch (Exception e) {
